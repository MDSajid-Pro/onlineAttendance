...
const Attendance = () => {
  ...
  const handleSave = async (e) => {
    ...
    const payload = {
      ...
      attendanceRecords: isHoliday
        ? []
        : students.filter((s) => s.status !== null),
    };
  ...
  try {
    ...
    // --- DATA EXTRACTION ---
    // Extracting students and holidays with fallbacks
    const studentsData = res.data.students || (Array.isArray(res.data) ? res.data : []);
    const monthlyHolidays = res.data.holidays || {}; 

    if (studentsData.length === 0) return alert("No data found for this selection.");
    ...
    doc.text(`${course} | Sem: ${semester} | ${monthName} ${year}`, 148, 22, { align: "center" });
    ...
    const tableRows = studentsData.map(s => {
      ...
      for (let i = 1; i <= daysInMonth; i++) {
        ...
        if (dayOfWeek === 0) row.push('SUN');
        else if (monthlyHolidays[dateKey]) row.push('HOL');
        else {
          const status = s.attendance?.[dateKey];
          row.push(status === 'present' ? 'P' : status === 'absent' ? 'A' : '-');
        }
      }
      row.push(s.totalPresent || 0);
      return row;
    });
    ...
    autoTable(doc, {
      ...
      didParseCell: (data) => {
        if (data.section === 'body') {
          ...
          if (val === 'SUN') data.cell.styles.fillColor = [255, 230, 230];
          if (val === 'HOL') data.cell.styles.fillColor = [230, 245, 255];
        }
      }
    });

    // --- THE FIX: DYNAMIC HOLIDAY REASONS ---
    // Ensure we start printing after the table ends
    let currentY = doc.lastAutoTable.finalY + 12;
    const holidayEntries = Object.entries(monthlyHolidays);

    if (holidayEntries.length > 0) {
      // If the table was very long, add a new page for the reasons
      if (currentY > 185) {
        doc.addPage();
        currentY = 20;
      }
      ...
      holidayEntries.forEach(([date, reason]) => {
        // Format: "15 Apr: Ambedkar Jayanti"
        const day = date.split('-')[2];
        const shortMonth = new Date(date).toLocaleString('en-US', { month: 'short' });
        
        doc.text(`• ${day} ${shortMonth}: ${reason}`, 16, currentY);
        currentY += 6;

        // Safety check for very long list of holidays
        if (currentY > 200) {
          doc.addPage();
          currentY = 20;
        }
    ...
    doc.save(`Attendance_${course}_${monthName}.pdf`);
  } catch (error) {
    console.error("PDF Export Error:", error);
    alert("Could not generate report. Check if data is loading correctly.");
  }
};
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <button
            ...
            className="flex items-center gap-2 bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-400 border border-emerald-500/30 px-5 py-2.5 rounded-2xl transition-all font-semibold"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 bg-white/5 p-4 rounded-3xl border border-white/10 backdrop-blur-xl">
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-slate-500 ml-2">
              COURSE
            </label>
            <select
              value={course}
              onChange={(e) => setCourse(e.target.value)}
              className="bg-slate-900 border-none rounded-2xl px-4 py-2.5 focus:ring-2 focus:ring-indigo-500 outline-none"
            >
              <option>B.Sc</option>
              <option>B.A</option>
              <option>B.Com</option>
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              className="bg-indigo-600 font-medium rounded-2xl px-4 py-2.5 outline-none"
            />
          </div>
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="bg-slate-900 w-full pl-10 pr-4 py-2.5 rounded-2xl border-none outline-none"
              />
            </div>
          </div>
        </div>
        ...
        {isHoliday && (
          <div className="bg-amber-500/10 border border-amber-500/30 p-4 rounded-3xl flex flex-col md:flex-row items-center gap-4 animate-in fade-in zoom-in duration-300">
            <p className="text-amber-500 font-semibold whitespace-nowrap">
              Reason for Holiday:
            </p>
            <input
              type="text"
        ...
        <div className="bg-white/5 rounded-4xl border border-white/10 overflow-hidden backdrop-blur-md">
          {isHoliday ? (
            <div className="h-64 flex flex-col items-center justify-center gap-4 text-center">
              ...
              <div>
                <h3 className="text-xl font-bold text-amber-500">
                  {holidayReason || "General Holiday"}
                </h3>
                <p className="text-slate-400">
                  Attendance records are disabled for this date.
                </p>
              </div>
            </div>
          ) : loading ? (
            <div className="h-64 flex flex-col items-center justify-center gap-4">
              ...
              <p className="text-slate-400 font-medium">Syncing...</p>
            </div>
          ) : (
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-white/5 text-slate-400 text-xs uppercase tracking-[0.2em]">
                    <th className="px-8 py-5">Register Number</th>
                ...
                <tbody className="divide-y divide-white/5">
                  {filteredStudents.length > 0 ? (
                    filteredStudents.map((student) => (
                      ...
                      >
                        <td className="px-8 py-5 font-mono text-indigo-400">
                          {student.registerNo}
                        </td>
                        ...
                        <td className="px-8 py-5">
                          <div className="flex justify-center gap-4">
                            <button
                              onClick={() =>
                                toggleStatus(student._id, "present")
                              }
                              className={`flex items-center gap-2 px-4 py-2 rounded-xl border transition-all ${student.status === "present" ? "bg-emerald-500 border-emerald-500 text-white shadow-lg" : "border-white/10 text-slate-400 hover:border-emerald-500"}`}
                            >
                              <UserCheck size={18} />{" "}
                            ...
                            <button
                              onClick={() =>
                                toggleStatus(student._id, "absent")
                              }
                              className={`flex items-center gap-2 px-4 py-2 rounded-xl border transition-all ${student.status === "absent" ? "bg-rose-500 border-rose-500 text-white shadow-lg" : "border-white/10 text-slate-400 hover:border-rose-500"}`}
                            >
                              <UserX size={18} />{" "}
          ...
          <div className="p-8 bg-white/2 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-4">
            {!isHoliday ? (
              <>
                ...
                <button
                  onClick={handleSave}
                  className="w-full md:w-auto flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white px-12 py-4 rounded-2xl font-bold shadow-xl transition-all"
                >
              ...
              <button
                onClick={handleSave}
                className="w-full flex items-center justify-center gap-2 bg-amber-600 hover:bg-amber-500 text-white py-4 rounded-2xl font-bold shadow-xl transition-all"
              >
                <Save size={20} /> Save Holiday ({holidayReason || "General"})
              </button>
            )}
          </div>
        </div>
